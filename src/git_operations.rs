...
fn get_ref_spec(branch_names: Vec<String>) -> String {
    let ref_spec = branch_names.into_iter().filter(|n| {
        n == "main" || n == "master"
    });
    ref_spec.collect()
}
fn get_head(repo: &Repository) -> Result<String, Error> {
    ...
    branches.fold(Err(git2::Error::new(git2::ErrorCode::GenericError,
                      git2::ErrorClass::Repository, "Couldn't get branches")),
                      |acc,branch| {
            let b = branch?;
            if b.0.is_head() {
                let name = b.0.name()?;
...
pub fn config_repo_mirror(repo_path: &str) -> Result<(), Error> {
    ...
    match config {
        Ok(mut c) => {
                    c.set_bool("remote.origin.mirror", true)
...
pub fn set_repo_push_url(url: &str,
                     name: &str,
                     repo_path: &str) -> Result<(), Error> {
    let repo = Repository::open(repo_path)?;
...
pub fn add_repo_remote<'repo>(repo: &'repo Repository,
                   name: &str,
                   remote_url: &str) -> Result<Remote<'repo>, Error> {
    
    repo.remote(name,remote_url)
    
}

pub fn set_repo_remote_url(repo: &Repository, name: &str, url: &str) -> Result<(), Error> {
...
pub fn push_repo(repo: Repository) -> Result<(), Error> {
    ...
    r.push(&[format!("refs/heads/{}", get_head(&repo)?)],Some(&mut push_options))?;    
