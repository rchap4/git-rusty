...
use git_operations::{
    clone_repo, config_repo_mirror, push_repo, set_repo_remote_url,
};
use structopt::StructOpt;

use github_operations::{build_repo_params, create_repo, get_repo};
...
fn check_empty_target_dir(path: PathBuf) -> Option<bool> {
...
async fn main() -> Result<(), Error> {
    ...
    match check_empty_target_dir(PathBuf::from(&args.output_path)) {
        Some(b) => {
            if b {
                if let Ok(res) =
                    clone_repo(&args.repo_url, true, &args.output_path)
                {
    ...
    let new_repo_url = format!(
        "https://github.com/{}/{}.git",
        &args.username, &args.new_repo_name
    );
    ...
    let create_res = create_repo(
        &create_repo_url,
        &args.username,
        &args.github_pat,
        build_repo_params(&args.new_repo_name),
    )
    .await;
    ...
    if let Ok(r) = create_res {
        if r.status().is_success() {
            ...
            let res =
                get_repo(&get_repo_url, &args.username, &args.github_pat).await;
            //fix me unwrap
            push_repo(repo).unwrap();
            match res {
                Ok(r) => println!(
                    "Get new repo from GitHub success? {}",
                    r.status().is_success()
                ),
                Err(e) => {
                    println!("Get new repo from GitHub not success: {}", e)
                }
            }
        }
    }
